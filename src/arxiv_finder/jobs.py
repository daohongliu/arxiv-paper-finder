...
JOB_KINDS = ("pipeline", "fetch", "affiliations", "screen")
PROMPTS_BY_KIND = {
...
def update_progress(conn: sqlite3.Connection, job_id: int, done: int, total: int, current: str) -> None:
    conn.execute(
        "UPDATE jobs SET progress_json = ? WHERE id = ?",
        (json.dumps({"done": done, "total": total, "current": current}), job_id),
    )
    conn.commit()
